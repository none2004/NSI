...
class Triangle:
    ...
    def angle(self,P,Q,R):
        if Q==self.sommets[0]:
            q=acos((self.BC**2-self.CA**2-self.AB**2)/(2*self.CA**2*self.AB**2))
        elif Q==self.sommets[1]:
            q=acos((self.CA**2-self.BC**2-self.AB**2)/(2*self.BC**2*self.AB**2))
        else:
            q=acos((self.AB**2-self.BC**2-self.CA**2)/(2*self.BC**2*self.CA**2))
        return degrees(q)
